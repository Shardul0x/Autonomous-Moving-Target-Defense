...
import random, time, threading, subprocess, tempfile, os
...
if DOCKER_MODE:
    nginx_switch("webapp")  # ensure we start pointing at webapp
...
INFRA = {
    ...
    "honeypot_active": False, "docker_mode": DOCKER_MODE,
    "gateway_target": "webapp",
}
SERVICES = {
...
# ── SHIFT ENGINE ──────────────────────────────────────────────
# Track honeypot state separately so it doesn't flip back on every tick
...
def apply_shift(intensity: str, result: dict):
    ...
    if intensity == "SOFT":
        ...
        changed.append(f"Route {old_r} -> {INFRA['route']}")
        changed.append("All services relocated to new containers")

        # EMERGENCY: engage honeypot ONCE, keep it until attack stops
        if intensity == "EMERGENCY" and not _honeypot_engaged:
            ...
            changed.append("HONEYPOT ACTIVATED — attacker trapped in deception system")

    # Only restore webapp when attacker explicitly stopped (not on every tick)
    # This is handled by /api/attacker/stop route instead

    # Docker action
    if DOCKER_MODE and docker_action:
        ok = nginx_switch(docker_action)
        changed.append(f"[Docker nginx -> {docker_action}: {'OK' if ok else 'FAILED'}]")
...
def ai_loop():
    ...
    while True:
        ...
        if result["is_anomaly"] or result["anomaly_score"] > 40 or tick % 15 == 0:
            exp = generate_llm_explanation(result, obs)
            LLM_LOG.appendleft({
                "time": ts, "text": exp,
...
def start_attack():
    global _honeypot_engaged
    _honeypot_engaged = False  # reset so honeypot can engage fresh
    simulator.start_attack()
    EVENT_LOG.appendleft({"time":datetime.now().strftime("%H:%M:%S"),
        "intensity":"ALERT","score":0,"conf":0,
        "detail":"Attacker simulation started — auto-escalating through 5 phases","color":"red"})
...
def stop_attack():
    ...
    if DOCKER_MODE:
        ok = nginx_switch("webapp")
        print(f"[MTD] Restored webapp on stop: {'OK' if ok else 'FAILED'}")
    EVENT_LOG.appendleft({"time":datetime.now().strftime("%H:%M:%S"),
...
if __name__ == "__main__":
    ...
    print("  India Innovates 2026  |  PROTOTYPE")
    print("="*55)
    print(f"  Dashboard  ->  http://localhost:5000")
    print(f"  Gateway    ->  http://localhost:8080  (watch this tab!)")
    print(f"  Real app   ->  http://localhost:8081")
    print(f"  Honeypot   ->  http://localhost:8082")
    print(f"  Docker     :   {'LIVE - real nginx shifts enabled' if DOCKER_MODE else 'NOT RUNNING - run docker-compose up -d'}")
    print("="*55 + "\n")
